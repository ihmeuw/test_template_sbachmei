===============================
test_template_sbachmei
===============================

Research repository for the test_template_sbachmei project.
...
all necessary requirements as follows::
  ...
  :~$ conda create --name=test_template_sbachmei python=3.11
  ...conda will download python and base dependencies...
  :~$ conda activate test_template_sbachmei
  (test_template_sbachmei) :~$ git clone https://github.com/ihmeuw/test_template_sbachmei.git
  ...git will copy the repository from github and place it in your current directory...
  (test_template_sbachmei) :~$ cd test_template_sbachmei
  (test_template_sbachmei) :~$ pip install -e .
...
`pip install -e .` with the following::
  ...
  (test_template_sbachmei) :~$ pip install -r requirements.txt
...
install hdf tooling from within your environment like so::
  ...
  (test_template_sbachmei) :~$ conda install hdf5

The ``(test_template_sbachmei)`` that precedes your shell prompt will probably show
...
``src/test_template_sbachmei`` package directory:
...
- ``components``
  ...
  the test_template_sbachmei project. You should work with the
...
With this model specification file and your conda environment active, you can then run simulations by, e.g.::
   ...
   (test_template_sbachmei) :~$ simulate run -v /<REPO_INSTALLATION_DIRECTORY>/test_template_sbachmei/src/test_template_sbachmei/model_specifications/model_spec.yaml
